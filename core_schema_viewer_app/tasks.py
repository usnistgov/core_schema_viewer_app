...
def delete_old_sandbox_data_structures():
    ...
    try:
        # get older queries
        old_ds = [ds for ds in get_all_sandbox_data_structures()
                  if ds.id.generation_time < timezone.now() - timedelta(days=SANDBOX_STRUCTURES_MAX_DAYS_IN_DATABASE)]
        ...
        logger.error("An error occurred while deleting sandbox data structures ({}).".format(str(e)))
