...
class FormDefaultTemplate(forms.Form):
    ...
    def set_default(self, widget="select", empty_label=True):
        ...
        visible_template_schema_viewer_list = template_schema_viewer_api.get_all_by_visibility(True)
        self.fields["schema"].choices = [(element.template.id, element.template.display_name)
                                         for element
                                         in visible_template_schema_viewer_list]
